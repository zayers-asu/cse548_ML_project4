...
categoryMappingsPath='mappings_scenario2/'
# Batch Size
BatchSize=10
# Epohe Size
NumEpoch=10
...
#%%
# ZACH: Join Datasets before sending to processed data so One hot encoding works:
fullFileName = "Full-Scenario3.csv"
test_db_size = len(dataset_test)
train_db_size = len(dataset_train)
print("Test db size: ", test_db_size, "; Train db size: ", train_db_size,"; sum: ",test_db_size+train_db_size)
dataset = dataset_train.append(dataset_test)
print("Total DB size: ?", len(dataset) )
np.savetxt(fullFileName, dataset, delimiter=',', fmt="%s" )

X_full, y_full = dp.get_processed_data(fullFileName, categoryMappingsPath)

# ZACH: split datasets back into their testing and training parts to use in neural network
print(X_full)
print(y_full)
print("X_full size: ",len(X_full),";y_full size: ",len(y_full))
X_train = X_full[0:train_db_size]
X_test = X_full[train_db_size:]
y_train = y_full[0:train_db_size]
y_test = y_full[train_db_size:]
...
plt.title('Senario C : model accuracy')
plt.ylabel('accuracy')
plt.xlabel('epoch')
...
plt.title('Scenario C : model loss')
plt.ylabel('loss')
plt.xlabel('epoch')
plt.legend(['train'], loc='upper left')
...
loss, accuracy = classifier.evaluate(X_test, y_pred)
print('Print the loss and the accuracy of the model on the dataset')
print('Loss [0,1]: %.4f' % (loss), 'Accuracy [0,1]: %.4f' % (accuracy))
